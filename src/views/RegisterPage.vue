<template>
  <div class="text-center">
    <form class="form-signin" @submit.prevent="userRegister">
      ...
      <h1 class="h3 mb-3 font-weight-normal text-muted">Register Storepedia</h1>
      ...
      <div class="mt-3">
          <p class="text-muted">Already have Storepedia account? <a href="/" @click.prevent="changeView('/login')">Login</a></p>
      </div>
  ...
  methods: {
    async userRegister () {
      try {
        ...
        const registered = await this.$store.dispatch('register', payload)
        if (registered) {
          Toast.fire({
            icon: 'success',
